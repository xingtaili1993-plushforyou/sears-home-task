...
async def list_technicians(db: Session = Depends(get_db)):
...
async def search_technicians(
    ...
    technicians = service.find_technicians_by_criteria(
        zip_code=zip_code, appliance_type=appliance_type, target_date=target_date
    )
...
async def get_appointment_by_confirmation(
    confirmation_number: str, db: Session = Depends(get_db)
):
