...
class SchedulingService:
    ...
    def get_available_slots(
        ...
        query = (
            self.db.query(TimeSlot, Technician)
            .join(Technician)
    ...
    def get_appointment_by_confirmation(
        ...
        return (
            ...
            .filter(Appointment.confirmation_number == confirmation_number)
            .first()
        )

    def cancel_appointment(self, appointment_id: int) -> Tuple[bool, Optional[str]]:
        """
    ...
    def get_appointments_for_customer(
        self, customer_id: int, include_past: bool = False
    ) -> List[Appointment]:
        """Get all appointments for a customer."""
        query = (
            self.db.query(Appointment)
            .options(
                joinedload(Appointment.technician), joinedload(Appointment.time_slot)
            )
            .filter(Appointment.customer_id == customer_id)
        )

        if not include_past:
            query = query.join(TimeSlot).filter(TimeSlot.date >= date.today())
