...
class ImageService:
    ...
    def create_upload_request(
        ...
        expires_at = datetime.utcnow() + timedelta(
            hours=settings.upload_url_expiry_hours
        )
    ...
    def get_upload_request_by_token(self, token: str) -> Optional[ImageUploadRequest]:
        """Get an upload request by its token."""
        return (
            self.db.query(ImageUploadRequest)
            .filter(ImageUploadRequest.upload_token == token)
            .first()
        )
    ...
    async def save_uploaded_image(
        self, token: str, image_data: bytes, filename: str
    ...
    async def analyze_image(self, token: str) -> Tuple[bool, Optional[str]]:
        """
