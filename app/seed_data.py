...
def create_time_slots(
    db: Session, technician: Technician, days_ahead: int = 14
) -> List[TimeSlot]:
    ...
    for tech_data in TECHNICIANS:
        ...
        for i, zip_code in enumerate(tech_data["zip_codes"]):
            service_area = TechnicianServiceArea(
                technician_id=technician.id, zip_code=zip_code, is_primary=(i == 0)
            )
            db.add(service_area)
...
def clear_and_reseed(db: Session) -> None:
    ...
    db.execute("DELETE FROM technician_specialty_association")
