...
class SessionManager:
    ...
    async def create_session(
        self, call_sid: str, customer_phone: str, customer_id: Optional[int] = None
    ) -> ConversationState:
    ...
    async def transition_phase(
        self, call_sid: str, new_phase: ConversationPhase
        ...
        if session:
            ...
            logger.info(f"Session {call_sid}: {old_phase.value} -> {new_phase.value}")
        return session
